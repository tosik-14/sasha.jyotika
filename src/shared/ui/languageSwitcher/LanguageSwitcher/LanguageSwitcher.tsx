...
export const LanguageSwitcher = () => {
    ...
    const { changeLanguage, setOpen, open, switcherRef } = useLanguageSwitcher();

    return (
        <div ref={switcherRef} className={styles.langSwitcher}>
            <button onClick={() => setOpen(!open)} className={styles.langSwitcherButton}><Icon name={"globalPlanet"} size={36} alr={"Language"}/></button>

            {open && (
                <ul className={styles.langList}>
                    {languages.map(({code, label}) => (
                        <li key={code}>
                            <button onClick={() => changeLanguage(code)} className={styles.langListLiButton}>
                                {label}
                            </button>
                        </li>
                    ))}
